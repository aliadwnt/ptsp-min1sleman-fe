...
<div className="flex items-center justify-center space-x-2 mb-4">
                  ...
                  <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead className="bg-gray-50 dark:bg-gray-800">
                      <tr>
                        ...
                        <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Ditujukan Kepada</th>
                        <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Isi Ringkas</th>
  ...
  <button onClick={() => { setCurrentSuratKeluar(item); setModalOpen(true); }} className="text-green-600 hover:text-green-900">
  ...
  <button onClick={() => handleDelete(item.id)} className="text-red-600 hover:text-red-900">
    <i className="fas fa-trash"></i>
                        ...
                        <tr>
                          <td colSpan="6" className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">No data available</td>
                        </tr>
                      )}
                    </tbody>
                  </table>
          ...
          {modalOpen && (
            <div className="fixed inset-0 flex items-center justify-center z-50">
              <div className="bg-white rounded-lg shadow-lg p-6 w-50">
                <h2 className="text-xl font-semibold mb-4">{currentSuratKeluar ? "Edit Surat Keluar" : "Tambah Surat Keluar"}</h2>
                <form onSubmit={handleSubmit}>
                  <input type="text" name="no_surat" defaultValue={currentSuratKeluar?.no_surat || ""} placeholder="No Surat" required className="block w-full p-2 border border-gray-300 rounded mb-4" />
                  <input type="text" name="ditujukan" defaultValue={currentSuratKeluar?.ditujukan || ""} placeholder="Ditujukan Kepada" required className="block w-full p-2 border border-gray-300 rounded mb-4" />
                  <input type="text" name="isi_ringkas" defaultValue={currentSuratKeluar?.isi_ringkas || ""} placeholder="Isi Ringkas" required className="block w-full p-2 border border-gray-300 rounded mb-4" />
                  {/* ubah input text menjadi ikon download */}
                  <input type="text" name="download" defaultValue={currentSuratKeluar?.download || ""} placeholder="Download" required className="block w-full p-2 border border-gray-300 rounded mb-4" />  
                  <div className="flex justify-end space-x-2">
                    <button type="button" onClick={handleModalClose} className="bg-gray-300 text-gray-700 px-4 py-2 rounded">Batal</button>
                    <button type="submit" className="bg-green-600 text-white px-4 py-2 rounded">{currentSuratKeluar ? "Update" : "Tambah"}</button>
                  </div>
                </form>
              </div>
            </div>
